...
$DATABASE_USER = 'root';
$DATABASE_PASS = '';
$DATABASE_NAME = 'sixth_star_forum';
try {
...
function show_comments($comments, $parent_id = -1) {
    ...
    foreach ($comments as $comment) {
        if ($comment['parent_id'] == $parent_id) {
            ...
            $html .= '
            <div class="comment" id="cmt_questions">
                <div>
                    <h6 class="name">' . htmlspecialchars($comment['name'], ENT_QUOTES) . '<span class="date" id="cmt_Timing">' . time_elapsed_string($comment['submit_date']) . '</span></h6>
...
function show_write_comment_form($parent_id = -1) {
    ...
    <div class="write_comment" data-comment-id="' . $parent_id . '" >
        <form id="cmt">
            ...
            <input name="name" type="text" placeholder="Your Name" required class="form-control">
            <textarea name="content" class="form-control"  rows="4"
            style="background: #fff;" placeholder="Ask your Query here..." required></textarea>
            <button type="submit" class="btn btn-primary btn-sm float-right">Submit Query</button>
        </form>
    </div>
    ';
    return $html;
...
<div class="comment_header">
    <span class="badge badge-warning total" style="color:#fff"><?=$comments_info['total_comments']?> comments</span>
    <!-- <a href="#" class="write_comment_btn" data-comment-id="-1">Ask Query</a> -->
</div>
